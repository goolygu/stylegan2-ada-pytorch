...
def generate_mtd_video(
    ...
    min_origin_img_dim = min(img.shape[1:3])
    img_dim = min(min_origin_img_dim, max_img_size)
    img_ratio = img_dim / min_origin_img_dim
    img_shape = (
        np.round(np.array(img.shape[1:3]) * img_ratio).astype(int).tolist()
        + list(img.shape[3:])
    )
    ...
    for i in range(num_waypoints):
        ...
        for j in tqdm(range(num_points_dim_0)):
            ...
            for k in range(num_points_dim_1):
                ...
                image = cv2.resize(image, img_shape[1::-1])
